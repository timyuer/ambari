...
if kerberos_security_enabled and stack_version_formatted != "" and 'kafka_principal_name' in config['configurations']['kafka-env'] \
  and check_stack_feature(StackFeature.KAFKA_KERBEROS, stack_version_formatted):
    ...
    kafka_user_keytab_path = config['configurations']['kafka-env']['kafka_user_keytab']
    kafka_user_principal_name = config['configurations']['kafka-env']['kafka_user_principal']
  ...
  kafka_kerberos_params = "-Djava.security.auth.login.config="+ conf_dir +"/kafka_jaas.conf"
    ...
    kafka_user_keytab_path = None
    kafka_user_principal_name = None
...
kafka_broker_port = default('/configurations/kafka-broker/port', None)

# get comma separated lists of kafka_broker hosts from clusterHostInfo
index = 0
kafka_broker_quorum = ""
for host in kafka_broker_hosts:
  kafka_broker_host = host
  if kafka_broker_port is not None:
    kafka_broker_host = host + ":" + str(kafka_broker_port)

  kafka_broker_quorum += kafka_broker_host
  index += 1
  if index < len(kafka_broker_hosts):
    kafka_broker_quorum += ","
...
if enable_ranger_kafka and is_supported_kafka_ranger:
  ...
  kafka_keytab_path_ranger = kafka_user_keytab_path if kafka_user_keytab_path else 'empty'
  kafka_jaas_principal_ranger = kafka_user_principal_name if kafka_user_principal_name else 'empty'
  ...
  ranger_plugin_config = {
    ...
    'bootstrap.servers' : kafka_broker_quorum,
    'security.protocol' : config['configurations']['kafka-broker']['security.inter.broker.protocol'],
    'kafka.keytab' : kafka_keytab_path_ranger,
    'kafka.principal' : kafka_jaas_principal_ranger,
    'sasl.mechanism' : config['configurations']['kafka-broker']['sasl.mechanism.inter.broker.protocol']
  }
  ...
  if kerberos_security_enabled:
    ...
    ranger_plugin_config['kafka.keytab'] =  kafka_keytab_path_ranger
    ranger_plugin_config['kafka.principal'] = kafka_jaas_principal_ranger
    ranger_plugin_config['sasl.mechanism'] = config['configurations']['kafka-broker']['sasl.mechanism.inter.broker.protocol']
